...
int Pedido::retirarProduto(string nome){ // Método para remoção de pedidos da lista.
    int qntd = 0;
    for(unsigned i(0); i < pedidos.size(); i++){ // Repetindo para a lista com n pedidos
        if (nome == pedidos[i].nome){ // Caso a string carregada da main seja igual a contida no pedido de posição i do vetor:
            qntd = pedidos[i].quantidade; // a variavael qntd será igual a que está salva no pedido
            pedidos.erase(pedidos.begin()+i); // Irá excluir o pedido da posição i
            cout<<"Produto removido!"<<endl;
            return qntd; // retorna a quantidade retirada do pedido
        }
    }
    cout<<"Produto não encontrado!"<<endl; 
    return 0; // caso não seja encontrado o pedido, retorna 0 
}
